...
def validate_coordinates(lat: float, lon: float) -> None:
    ...
    if not (-90.0 <= lat <= 90.0):
        raise ValueError(
            f"Широта должна быть в диапазоне [-90, 90], получено: {lat}"
        )
...
def validate_humidity(rh: float) -> None:
    ...
    if not (0.0 <= rh <= 100.0):
        raise ValueError(
            f"Относительная влажность должна быть в диапазоне [0, 100]%, "
            f"получено: {rh}"
        )
...
def validate_wind_speed(v: float) -> None:
    ...
    if not (0.0 <= v <= 120.0):
        raise ValueError(
            f"Скорость ветра должна быть в диапазоне [0, 120] м/с, "
            f"получено: {v}"
        )